...
// Ye grid sirf Women's page ke liye hai — is liye `category` prop nahi
// leta, balke hamesha "women" fixed rehta hai. Poora page (fetch, filters,
// empty-state text, heading) sirf women ke products ke around hi ghumta
// hai — koi generic "men/women/accessories" wala prop switch nahi hai.
const CATEGORY = "women";

// Same product-worker endpoint jo men's page use kar raha hai, bas yahan
// query hamesha `category=women` bhejti hai.
const API_BASE =
...
// is liye agar women's category mein 50 se zyada products hon to hum
// pagination loop laga kar sab pages khींch lete hain.
async function fetchAllWomenProducts(signal) {
  const all = [];
  let page = 1;
  const limit = 50;
...
export default function WomenProductGrid({ userId: userIdProp }) {
  ...
  // Database se women's products laana hai — sirf mount pe aur jab
  ...
  // bhi latest UI ko overwrite na kare (jaise "No pieces found" ka
  // galat flash).
  useEffect(() => {
    ...
    fetchAllWomenProducts(controller.signal)
      .then((rows) => {
        if (cancelled) return;
  ...
  const filteredAndSortedProducts = useMemo(() => {
    // `products` yahan already sirf women's category ka data hai
    // (API call khud "category=women" bhejti hai), is liye neeche
    // sirf product `type` (dresses/tops/etc.) pe filter lagta hai —
    // koi men/accessories wala data is list mein aata hi nahi.
    let result = [...products];
        ...
        {isLoadingProducts ? (
          ...
          <div className="flex min-h-[460px] items-center justify-center">
            <div className="relative w-full max-w-xl overflow-hidden rounded-[2rem] border border-[#D8CFC2] bg-white/25 px-6 py-16 text-center shadow-[0_20px_60px_rgba(67,40,23,0.04)] backdrop-blur-sm sm:px-12">
              ...
              <h3 className="font-serif text-3xl tracking-[-0.02em] text-[#432817] sm:text-4xl">
                Couldn't load the women's collection
              </h3>
          ...
          <div className="flex min-h-[460px] items-center justify-center">
            <div className="relative w-full max-w-xl overflow-hidden rounded-[2rem] border border-[#D8CFC2] bg-white/25 px-6 py-16 text-center shadow-[0_20px_60px_rgba(67,40,23,0.04)] backdrop-blur-sm sm:px-12">
              ...
              <h3 className="font-serif text-3xl tracking-[-0.02em] text-[#432817] sm:text-4xl">
                No women's pieces found
              </h3>

              <p className="mx-auto mt-4 max-w-sm text-xs leading-6 text-[#7E7E86]">
                We couldn't find any products in the women's
